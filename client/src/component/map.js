...
const Map = () => {
  ...
  const initializeMap = () => {
    ...
    fetchNearbyHospitals(sortHospitalsByDistance, addMarkers);
  };

  const fetchNearbyHospitals = async (sortHospitalsByDistance, addMarkers) => {
    if (!userLocation) {
            ...
            {hospitals.map((hospital, index) => (
              <li key={index}>
                ...
                <p>
                  Distance:{" "}
                  {hospital.distance
                    ? `${(hospital.distance / 1609.34).toFixed(2)} mi`
                    : "N/A"}
                </p>
